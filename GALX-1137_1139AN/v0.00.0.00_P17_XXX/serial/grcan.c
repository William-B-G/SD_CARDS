...
//    Data_Zero = grcan_rx[grc_rgix].data[0]; // wbg 2018-3-12
...
//                SIU.GPDO[Z6_LED].R = !SIU.GPDO[Z6_LED].R;
	 			...
	 			switch (grcan_rx[grc_rgix].msg)
				{
					...
					case 14:	// message 14: device inputs
						...
						{	  // id1 is the address
	                        if(Enable_gr_CAN_Ctrl && ( devnmb   == Hall_1137_bd_Addr[test_Loop] ) )
	                        {  // Hall station ( GALX-1137AN )
//	                            SIU.GPDO[Z6_LED].R = !SIU.GPDO[Z6_LED].R;
	                            Data_Zero =     grcan_rx[grc_rgix].data[0]; // wbg 2018-3-12
	                            Case_14_data = grcan_rx[grc_rgix].data[0];
	                            if( GALX4_0047N )// toggled with menu 24
	                            {
	                                Toggle_LDOxx(Data_Zero) ;  
	                            }
	                            //sprintf(src,"grCAN case %d ",grcan_rx[grc_rgix].msg);
//	                            sprintf(src,"Data[0];%d, addr:%d ", Data_Zero, devnmb);
//	                            Log_an_event(src);
	                        }						
...
//            Add_msg_to_array_B(src,0); // let's see what the data looks like
		...
		case 15:		// I/O Board inputs sent back to the can serial i/o board
			 	...
			 	((devnmb >= c_grcan_SEB_1) && (devnmb <= c_grcan_SEB_12) && ((cons[sercom] & 0x01) != 0)))
			{
				...
				grcan_tx[grc_tpix].data[7] = 0;
				grcan_dev[devnmb].tx_cnt++;
//				if (grcan_dev[devnmb].online == 0) SIU.GPDO[Z6_LED].R = !SIU.GPDO[Z6_LED].R;  // toggle LED wbg
			}
			else if ((devnmb >= c_grcan_HCB_1) && (devnmb <= c_grcan_HCB_40) && ((cons[sercom] & 0x01) != 0))
			{	  // Individual hall call board
				grcan_tx[grc_tpix].rtr = 0;
