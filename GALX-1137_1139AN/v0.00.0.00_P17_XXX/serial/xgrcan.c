...
#define WBG_MAX_GRP_CAN_INDEX 12
int16 wbg_grp_pack_num[WBG_MAX_GRP_CAN_INDEX];
int16 wbg_grp_pack_index = 0;
int16 Total_Packs = 0;
int16 Hall_Station_Addr = 0;
...
int16 get_comm_status (int16 board_nmb)
{
	...
	{			// Individual Serial Hall call boards
		if (board_nmb >= HCB_IODATA_GRCAN_START)
		{
			devnmb = (board_nmb - HCB_IODATA_GRCAN_START) + c_grcan_HCB_1;
	...
	{			// Serial Hall call driver boards
		...
		{
			devnmb = (board_nmb/6) + c_grcan_HCDB_1;		
			comm_status = (grcan_dev[devnmb].online == 1);
		}
		else
			comm_status = 2;
...
void send_hc_dvr_req (int16 cmd)
{
	...
	while (i < nmb_grcan_dev) 
	{
		...
		{
			dev_ix = grcan_dev_list[i] - c_grcan_HCDB_1;
			if (cmd == 0)
	...
	for (i=0;i<nmb_hcio_brds;i++)			
	{
		...
		{		 // Set device in device list
			...
			{   	// Individual Serial Board for each floor
				if (j >= HCB_IODATA_GRCAN_START)
				{
					
					if ((gio_valid[i] & 0x0F) != 0)
					{
						grcan_dev_list[nmb_grcan_dev] = (uint8)(c_grcan_HCB_1 + (j - HCB_IODATA_GRCAN_START));
						nmb_grcan_dev ++;					  // ^^starts after c_grcan_SEB_12 so must index from c_grcan_SEB_1, IO array 36
					}
					serial_grc.brd_io[j] = GIO_In_Out[j];
					j++;
				}
				else
					j++;
			...
			{			// Serial Hall call driver boards
				if ((i%6) == 0) 	// using a driver board
				{
					...
					grcan_dev_list[nmb_grcan_dev] = (uint8)(c_grcan_HCDB_1 + h);
					nmb_grcan_dev ++;
					nmb_dvr_brds++;
					k=h*c_io_ix_mult;
...
void unpack_grcan_in (void)
{
    ...
    Raw_data = grcan_rx[grc_rgix].data[0]; // wbg 2018-3-12
    GRC_Rgix = grc_rgix; // wbg 2018-3-12
    grcan_rx[grc_rgix].data[0] = 55; // wbg 2018-3-12
//    SIU.GPDO[Z6_LED].R = !SIU.GPDO[Z6_LED].R; // wbg_01_2018_4_25
//    if(consB[29] & 0x2) SIU.GPDO[Z6_LED].R = !SIU.GPDO[Z6_LED].R; // wbg_01_2018_4_25
...
//		    SIU.GPDO[Z6_LED].R = !SIU.GPDO[Z6_LED].R; // wbg_01_2018_4_25	
			...
			{		// OK to unpack the data 
			 //////////////// wbg ///////////////////////////   
			    grcan_dev[devnmb].online = 1;
//			    sprintf(src,"unpk gr %d; %d: %d", 
//			        grcan_rx[grc_rgix].msg, devnmb, 
//			            grcan_dev[devnmb].online);
//                Add_msg_to_array_B(src, devnmb);
               // // SIU.GPDO[Z6_LED].R = 0;
              ////////////////////////////////////////  
//                SIU.GPDO[Z6_LED].R = !SIU.GPDO[Z6_LED].R;  // toggle LED wbg  // wbg_01_2018_4_25
/*
			wbg_grp_pack_num[wbg_grp_pack_index++] = grcan_rx[grc_rgix].msg;
			if(wbg_grp_pack_index > 11) wbg_grp_pack_index = 0;
			Total_Packs++;
*/			
//			SIU.GPDO[Z6_LED].R = !SIU.GPDO[Z6_LED].R; // wbg_01_2018_4_25
//			if(consB[29] & 0x4) SIU.GPDO[Z6_LED].R = !SIU.GPDO[Z6_LED].R; // wbg_01_2018_4_25
	 			switch (grcan_rx[grc_rgix].msg)
				{
					...
					case 14:	// message 14: device inputs
						...
						{	  // id1 is the address
						    grcan_dev[devnmb].online = 1; // must be online
//						    if(consB[29] & 0x8) SIU.GPDO[Z6_LED].R = !SIU.GPDO[Z6_LED].R; // wbg_01_2018_4_25
							iodev_ix = (int16)(devnmb-c_grcan_SEB_1);
						...
						{	  // Individual hall call board
							iodataix = (int16)(HCB_IODATA_GRCAN_START + (devnmb-c_grcan_HCB_1)); 		// input array
							iodev_ix = iodataix;
									// Set input data							
							serial_grc.hcb[iodev_ix].iob = (uint8)((serial_grc.hcb[iodev_ix].iob & GIO_In_Out[iodataix]) | ((grcan_rx[grc_rgix].data[0] & 0x0F) & ~GIO_In_Out[iodataix]));
							sync_grc.hcb[iodev_ix].reflect_data = 1;
							timers[tgrcanrx + (devnmb-c_grcan_SEB_1)] = 0;
							grcan_dev[devnmb].online_cnt = 5;
							grcan_dev[devnmb].rx_cnt++;
							Case_14_data = grcan_rx[grc_rgix].data[0];
					        sprintf(src,"Data;%d",Case_14_data);
//							Log_an_event(src);
//                            Add_msg_to_array_B(src,14);	
					...
					case 16:	// message 16: return of output data					    					    
						...
						{	  // Individual hall call board
//						    if(consB[29] & 0x10) SIU.GPDO[Z6_LED].R = !SIU.GPDO[Z6_LED].R; // wbg_01_2018_4_25
							iodataix = (int16)(HCB_IODATA_GRCAN_START + (devnmb-c_grcan_HCB_1)); 		// input array
							iodev_ix = iodataix;
							// Set output return data							
							sync_grc.hcb[iodev_ix].iob = (uint8)((sync_grc.hcb[iodev_ix].iob & ~GIO_In_Out[iodataix]) | (~(grcan_rx[grc_rgix].data[0] & 0x0F) & GIO_In_Out[iodataix]));
							grcan_dev[devnmb].online_cnt = 5;
							grcan_dev[devnmb].rx_cnt++;  
							sprintf(src,"data/dev;%d,%d",iodataix,iodev_ix);
//							Log_an_event(src);
//                          Add_msg_to_array_B(src,11); // let's see what the data looks like
							sprintf(src,"online/rx;%d,%d",grcan_dev[devnmb].online_cnt,grcan_dev[devnmb].rx_cnt);
//							Log_an_event(src);
//                            Add_msg_to_array_B(src,12); // let's see what the data looks like 
							sprintf(src,"sync;%d",sync_grc.hcb[iodev_ix].iob);
//							Log_an_event(src);
//                            Add_msg_to_array_B(src,13); 
                            Case_16_data = grcan_rx[grc_rgix].data[0];
							sprintf(src,"data; %d",Case_16_data);
//							Log_an_event(src);
//                            Add_msg_to_array_B(src,15);   
					...
					case 46:	// message 46: Request for i/o labels
						...
						{	  
							...
							Group_CAN_Test_OK =  1;// That's good (wbg)
						}
						break;
...
*/
					...
					case 74:	// message 74: device inputs
						if ((devnmb >= c_grcan_HCDB_1) && (devnmb <= c_grcan_HCDB_10))
						{	  // id1 is the address
							iodev_ix = (int16)(devnmb-c_grcan_HCDB_1);
							...
							sync_grc.dev[iodev_ix].reflect_data = 1;
							grcan_debug_2++;
							timers[tgrcanrx + (devnmb-c_grcan_HCDB_1)] = 0;
							grcan_dev[devnmb].online_cnt = 5;
							grcan_dev[devnmb].rx_cnt++;
						}
						break;
...
//    Add_msg_to_array_B(src, 2);
	...
	wbg_grp_pack_num[wbg_grp_pack_index++] = msg;
	if(wbg_grp_pack_index > 11) wbg_grp_pack_index = 0;
	Total_Packs = Total_Packs + 1;  // Total_Packs++;   
	switch (msg)
	{
		...
		case 13:		// I/O Board outputs
		    sprintf(src,"dev#;sercom %d;%d",devnmb,sercom );
            Add_msg_to_array_B(src,0); // let's see what the data looks like
			if (((devnmb >= c_grcan_SEB_1) && (devnmb <= c_grcan_SEB_40) && ((cons[sercom] & 0x01) == 0)) ||
			 	((devnmb >= c_grcan_SEB_1) && (devnmb <= c_grcan_SEB_12) && ((cons[sercom] & 0x01) != 0)))
			{			    
				grcan_tx[grc_tpix].rtr = 0;
				...
				grcan_tx[grc_tpix].msg = msg;
				grcan_tx[grc_tpix].addr = (uint8)grcan_addr[devnmb];
				Hall_Station_Addr = (uint8)grcan_addr[devnmb];
//                SIU.GPDO[Z6_LED].R = !SIU.GPDO[Z6_LED].R;  // toggle LED wbg  // wbg_01_2018_4_25				
				grcan_tx[grc_tpix].id2 = 0;
				grcan_tx[grc_tpix].id3 = 0;
				iodev_ix = (int16)(devnmb-c_grcan_SEB_1);
			...
			{	  // Individual hall call board
				...
				grcan_tx[grc_tpix].id2 = 0;
				grcan_tx[grc_tpix].id3 = 0;
				iodataix = (int16)(HCB_IODATA_GRCAN_START + (devnmb-c_grcan_HCB_1)); 		// i/o array
				iodev_ix = iodataix;
				// Set input data							
				grcan_tx[grc_tpix].data[0] = (uint8)(~serial_grc.hcb[iodev_ix].iob & GIO_In_Out[iodataix] & 0x0F);
				grcan_tx[grc_tpix].data[1] = 0;
				grcan_tx[grc_tpix].data[2] = 0;
				grcan_tx[grc_tpix].data[3] = 0;
				grcan_tx[grc_tpix].data[4] = 0;
	...
	if (grc_tgix != grc_tpix)
	{
		...
		{			
			case 8:
			    ...
			    sprintf(src,"Code/tx %d;%d", GR_CAN.MB[TxBufIx].MSG.MCS.B.CODE, grcan_tx_complete[TxBufIx]);
                Add_msg_to_array_B(src,3); // let's see what the data looks like
...
//        SIU.GPDO[Z6_LED].R = !SIU.GPDO[Z6_LED].R;  // toggle LED wbg  // wbg_01_2018_4_25
        ...
        sprintf(src,"tgix/siz %d;%d",grc_tgix, c_grcan_tx_buf_siz);
        Add_msg_to_array_B(src,4); // let's see what the data looks like
		...
		sprintf(src,"len/ex_id %d;%d",grcan_tx[grc_tgix].len, grcan_tx[grc_tgix].ex_id);
        Add_msg_to_array_B(src,5); // let's see what the data looks like
		...
		sprintf(src,"msg/addr %d;%d",grcan_tx[grc_tgix].msg, grcan_tx[grc_tgix].addr);
        Add_msg_to_array_B(src,6); // let's see what the data looks like
...
void grcan_control (void)
{
	...
	uint8 devix,devnmb,tdevix;
	uint16 iodev_ix;
	uint16 iodataix;
//    if(consB[29] & 0x1) SIU.GPDO[Z6_LED].R = !SIU.GPDO[Z6_LED].R;  // toggle LED wbg  // wbg_01_2018_4_25
//    SIU.GPDO[Z6_LED].R = !SIU.GPDO[Z6_LED].R;  // toggle LED wbg  // wbg_01_2018_4_25
    sprintf(src,"nmb_grcan_dev;%d",nmb_grcan_dev);
    Add_msg_to_array_B(src,2); // let's see what the data looks like
	if (nmb_grcan_dev > 0)
	{
		unpack_grcan_in();
...
//			record_fault(f_grcan_ack_err);	
		...
		if (Dispatcher == cons[carnmb])
		{
			...
			while ((i < nmb_grcan_dev) && (IsGrcTxBufFul() == 0))
			{
				...
				{		 // Valid device number
					...
					if (((giodata(iodataix+0) & GIO_In_Out[iodataix+0]) != (serial_grc.dev[iodev_ix].iob[0] & GIO_In_Out[iodataix+0])) ||
						((giodata(iodataix+1) & GIO_In_Out[iodataix+1]) != (serial_grc.dev[iodev_ix].iob[1] & GIO_In_Out[iodataix+1])) ||
						((giodata(iodataix+2) & GIO_In_Out[iodataix+2]) != (serial_grc.dev[iodev_ix].iob[2] & GIO_In_Out[iodataix+2]))) 
					{		// io data for the output has changed
						serial_grc.dev[iodev_ix].iob[0] = (uint8)((serial_grc.dev[iodev_ix].iob[0] & ~GIO_In_Out[iodataix+0]) | (giodata(iodataix+0) & GIO_In_Out[iodataix+0]));
						serial_grc.dev[iodev_ix].iob[1] = (uint8)((serial_grc.dev[iodev_ix].iob[1] & ~GIO_In_Out[iodataix+1]) | (giodata(iodataix+1) & GIO_In_Out[iodataix+1]));
						serial_grc.dev[iodev_ix].iob[2] = (uint8)((serial_grc.dev[iodev_ix].iob[2] & ~GIO_In_Out[iodataix+2]) | (giodata(iodataix+2) & GIO_In_Out[iodataix+2]));
						pack_grcan_out(13,devnmb,0);	// Data has changed so pack the data
						timers[tgrcantx + tdevix] = 0;
						sync_grc.dev[iodev_ix].rtx_cnt = 0;
					}
					else if (((sync_grc.dev[iodev_ix].iob[0] & GIO_In_Out[iodataix+0]) != (serial_grc.dev[iodev_ix].iob[0] & GIO_In_Out[iodataix+0])) ||
							 ((sync_grc.dev[iodev_ix].iob[1] & GIO_In_Out[iodataix+1]) != (serial_grc.dev[iodev_ix].iob[1] & GIO_In_Out[iodataix+1])) ||
							 ((sync_grc.dev[iodev_ix].iob[2] & GIO_In_Out[iodataix+2]) != (serial_grc.dev[iodev_ix].iob[2] & GIO_In_Out[iodataix+2]))) 
					{		// device has not updated it's outputs yet
						if (timers[tgrcantx + tdevix] >= 2)
						{
							pack_grcan_out(13,devnmb,0);	// Data has changed so pack the data
							timers[tgrcantx + tdevix] = 0;
							if (sync_grc.dev[iodev_ix].rtx_cnt > 10)
							{
								sync_grc.dev[iodev_ix].iob[0] = (uint8)((sync_grc.dev[iodev_ix].iob[0] & ~GIO_In_Out[iodataix+0]) | (serial_grc.dev[iodev_ix].iob[0] & GIO_In_Out[iodataix+0]));
								sync_grc.dev[iodev_ix].iob[1] = (uint8)((sync_grc.dev[iodev_ix].iob[1] & ~GIO_In_Out[iodataix+1]) | (serial_grc.dev[iodev_ix].iob[1] & GIO_In_Out[iodataix+1]));
								sync_grc.dev[iodev_ix].iob[2] = (uint8)((sync_grc.dev[iodev_ix].iob[2] & ~GIO_In_Out[iodataix+2]) | (serial_grc.dev[iodev_ix].iob[2] & GIO_In_Out[iodataix+2]));
								sync_grc.dev[iodev_ix].rtx_cnt = 0;
							}
							else
								sync_grc.dev[iodev_ix].rtx_cnt++;
						}
					}
					...
					if (sync_grc.dev[iodev_ix].reflect_data == 1)
					{		 // Send back what was sent so the device can verify it was sent ok
						sync_grc.dev[iodev_ix].reflect_data = 0;	// Must be cleared before data is packed - if new data comes in during
						grcan_debug_3++;
	...
	//						record_fault(f_seb_grcan_error);
						...
						grcan_dev[devix].online = 0;
#if (Simulator == 0)
			 			serial_grc.dev[iodev_ix].iob[0] &= GIO_In_Out[iodataix+0];	  // clear input data since device is not on-line
			 			serial_grc.dev[iodev_ix].iob[1] &= GIO_In_Out[iodataix+1];
			 			serial_grc.dev[iodev_ix].iob[2] &= GIO_In_Out[iodataix+2];
#endif
					}
				}
				...
				if ((devnmb >= c_grcan_HCDB_1) && (devnmb <= c_grcan_HCDB_10) && ((cons[sercom] & 0x01) == 0))
				{		 // Valid device number
					iodev_ix = (int16)(devnmb-c_grcan_HCDB_1);
					tdevix = devnmb - c_grcan_HCDB_1;
					iodataix = (int16)((IODATA_GRCAN_START + iodev_ix) * c_io_ix_mult); 		// input or output array

					// *********************** Output device *********************
						 
					if (((giodata(iodataix+6) & GIO_In_Out[iodataix+6]) != (serial_grc.devhc[iodev_ix].ohc[0] & GIO_In_Out[iodataix+6])) ||
						((giodata(iodataix+7) & GIO_In_Out[iodataix+7]) != (serial_grc.devhc[iodev_ix].ohc[1] & GIO_In_Out[iodataix+7])) ||
						((giodata(iodataix+8) & GIO_In_Out[iodataix+8]) != (serial_grc.devhc[iodev_ix].ohc[2] & GIO_In_Out[iodataix+8])) ||
						((giodata(iodataix+9) & GIO_In_Out[iodataix+9]) != (serial_grc.devhc[iodev_ix].ohc[3] & GIO_In_Out[iodataix+9])) ||
						((giodata(iodataix+10) & GIO_In_Out[iodataix+10]) != (serial_grc.devhc[iodev_ix].ohc[4] & GIO_In_Out[iodataix+10])) ||
						((giodata(iodataix+11) & GIO_In_Out[iodataix+11]) != (serial_grc.devhc[iodev_ix].ohc[5] & GIO_In_Out[iodataix+11])))
					{		// io data for the output has changed
						serial_grc.devhc[iodev_ix].ohc[0] = (uint8)((serial_grc.devhc[iodev_ix].ohc[0] & ~GIO_In_Out[iodataix+6]) | (giodata(iodataix+6) & GIO_In_Out[iodataix+6]));
						serial_grc.devhc[iodev_ix].ohc[1] = (uint8)((serial_grc.devhc[iodev_ix].ohc[1] & ~GIO_In_Out[iodataix+7]) | (giodata(iodataix+7) & GIO_In_Out[iodataix+7]));
						serial_grc.devhc[iodev_ix].ohc[2] = (uint8)((serial_grc.devhc[iodev_ix].ohc[2] & ~GIO_In_Out[iodataix+8]) | (giodata(iodataix+8) & GIO_In_Out[iodataix+8]));
						serial_grc.devhc[iodev_ix].ohc[3] = (uint8)((serial_grc.devhc[iodev_ix].ohc[3] & ~GIO_In_Out[iodataix+9]) | (giodata(iodataix+9) & GIO_In_Out[iodataix+9]));
						serial_grc.devhc[iodev_ix].ohc[4] = (uint8)((serial_grc.devhc[iodev_ix].ohc[4] & ~GIO_In_Out[iodataix+10]) | (giodata(iodataix+10) & GIO_In_Out[iodataix+10]));
						serial_grc.devhc[iodev_ix].ohc[5] = (uint8)((serial_grc.devhc[iodev_ix].ohc[5] & ~GIO_In_Out[iodataix+11]) | (giodata(iodataix+11) & GIO_In_Out[iodataix+11]));
						pack_grcan_out(73,devnmb,0);	// Data has changed so pack the data
						timers[tgrcantx + tdevix] = 0;
						sync_grc.devhc[iodev_ix].rtx_hc_cnt = 0;
					}
					
					if (((giodata(iodataix+15) & GIO_In_Out[iodataix+15]) != (serial_grc.devhc[iodev_ix].axohc[0] & GIO_In_Out[iodataix+15])) ||
						((giodata(iodataix+16) & GIO_In_Out[iodataix+16]) != (serial_grc.devhc[iodev_ix].axohc[1] & GIO_In_Out[iodataix+16])) ||
						((giodata(iodataix+17) & GIO_In_Out[iodataix+17]) != (serial_grc.devhc[iodev_ix].axohc[2] & GIO_In_Out[iodataix+17])))
					{		// io data for the output has changed
						serial_grc.devhc[iodev_ix].axohc[0] = (uint8)((serial_grc.devhc[iodev_ix].axohc[0] & ~GIO_In_Out[iodataix+15]) | (giodata(iodataix+15) & GIO_In_Out[iodataix+15]));
						serial_grc.devhc[iodev_ix].axohc[1] = (uint8)((serial_grc.devhc[iodev_ix].axohc[1] & ~GIO_In_Out[iodataix+16]) | (giodata(iodataix+16) & GIO_In_Out[iodataix+16]));
						serial_grc.devhc[iodev_ix].axohc[2] = (uint8)((serial_grc.devhc[iodev_ix].axohc[2] & ~GIO_In_Out[iodataix+17]) | (giodata(iodataix+17) & GIO_In_Out[iodataix+17]));
						if (((hc_dvr_brd[iodev_ix].riser == 2) && (cons[cblue] == 2)) || 
							((hc_dvr_brd[iodev_ix].riser == 3) && (cons[VipSvc] == 4)))
					...
					{		 // Update outputs
						...
						if (hc_dvr_brd[iodev_ix].upd_seq == 0)
						{
							hc_dvr_brd[iodev_ix].upd_seq = 1;
							serial_grc.devhc[iodev_ix].ohc[0] = (uint8)((serial_grc.devhc[iodev_ix].ohc[0] & ~GIO_In_Out[iodataix+6]) | (giodata(iodataix+6) & GIO_In_Out[iodataix+6]));
							serial_grc.devhc[iodev_ix].ohc[1] = (uint8)((serial_grc.devhc[iodev_ix].ohc[1] & ~GIO_In_Out[iodataix+7]) | (giodata(iodataix+7) & GIO_In_Out[iodataix+7]));
							serial_grc.devhc[iodev_ix].ohc[2] = (uint8)((serial_grc.devhc[iodev_ix].ohc[2] & ~GIO_In_Out[iodataix+8]) | (giodata(iodataix+8) & GIO_In_Out[iodataix+8]));
							serial_grc.devhc[iodev_ix].ohc[3] = (uint8)((serial_grc.devhc[iodev_ix].ohc[3] & ~GIO_In_Out[iodataix+9]) | (giodata(iodataix+9) & GIO_In_Out[iodataix+9]));
						...
						else if (hc_dvr_brd[iodev_ix].upd_seq == 1)
						{
							hc_dvr_brd[iodev_ix].upd_seq = 2;
							pack_grcan_out(72,devnmb,0);	// control update
							
							if ((cons[cblue] == 0) && (cons[VipSvc] == 0))
							{
								hc_dvr_brd[iodev_ix].upd_seq = 0;
								timers[tgrcantx + tdevix] = 0;
							}
						}
						else if (hc_dvr_brd[iodev_ix].upd_seq == 2)
						{
							hc_dvr_brd[iodev_ix].upd_seq = 0;
							if (((hc_dvr_brd[iodev_ix].riser == 2) && (cons[cblue] == 2)) || 
								((hc_dvr_brd[iodev_ix].riser == 3) && (cons[VipSvc] == 4)))
...
/*					if (sync_grc.devhc[iodev_ix].reflect_hc_data == 1)
					{		 // Send back what was sent so the device can verify it was sent ok
						sync_grc.devhc[iodev_ix].reflect_hc_data = 0;	// Must be cleared before data is packed - if new data comes in during
						grcan_debug_3++;
	...
	//						record_fault(f_seb_grcan_error);
						...
						grcan_dev[devix].online = 0;
						hc_dvr_brd[iodev_ix].init_seq = 0;
						hc_dvr_brd[iodev_ix].init_count = 0;
						hc_dvr_brd[iodev_ix].online = 0;
#if (Simulator == 0)
						iodataix = (int16)((IODATA_GRCAN_START + iodev_ix) * c_io_ix_mult); 		// input or output array
			 			serial_grc.devhc[iodev_ix].ihc[0] &= GIO_In_Out[iodataix+0];	  // clear input data since device is not on-line
			 			serial_grc.devhc[iodev_ix].ihc[1] &= GIO_In_Out[iodataix+1];
			 			serial_grc.devhc[iodev_ix].ihc[2] &= GIO_In_Out[iodataix+2];
			 			serial_grc.devhc[iodev_ix].ihc[3] &= GIO_In_Out[iodataix+3];
			 			serial_grc.devhc[iodev_ix].ihc[4] &= GIO_In_Out[iodataix+4];
			 			serial_grc.devhc[iodev_ix].ihc[5] &= GIO_In_Out[iodataix+5];
			 			serial_grc.devhc[iodev_ix].axihc[0] &= GIO_In_Out[iodataix+12];
			 			serial_grc.devhc[iodev_ix].axihc[1] &= GIO_In_Out[iodataix+13];
			 			serial_grc.devhc[iodev_ix].axihc[2] &= GIO_In_Out[iodataix+14];
#endif
					}
				}
				...
				{		 // Valid device number
					...
					if ((giodata(iodataix) & GIO_In_Out[iodataix]) != (serial_grc.hcb[iodev_ix].iob & GIO_In_Out[iodataix]))
					{		// pack data out
					   	serial_grc.hcb[iodev_ix].iob = (uint8)((serial_grc.hcb[iodev_ix].iob & ~GIO_In_Out[iodataix]) | (giodata(iodataix) & GIO_In_Out[iodataix]));
						pack_grcan_out(13,devnmb,0);	// Data has changed so pack the data
						timers[tgrcantx + tdevix] = 0;
						sync_grc.hcb[iodev_ix].rtx_cnt = 0;
					}
					else if ((sync_grc.hcb[iodev_ix].iob & GIO_In_Out[iodataix]) != (serial_grc.hcb[iodev_ix].iob & GIO_In_Out[iodataix]))
					{		// device has not updated it's outputs yet
						if (timers[tgrcantx + tdevix] >= 2)
						{
							pack_grcan_out(13,devnmb,0);	// Data has changed so pack the data
							timers[tgrcantx + tdevix] = 0;
							if (sync_grc.hcb[iodev_ix].rtx_cnt > 10)
							{
								sync_grc.hcb[iodev_ix].iob = (uint8)((sync_grc.hcb[iodev_ix].iob & ~GIO_In_Out[iodataix]) | (serial_grc.hcb[iodev_ix].iob & GIO_In_Out[iodataix]));
								sync_grc.hcb[iodev_ix].rtx_cnt = 0;
							}
							else
								sync_grc.hcb[iodev_ix].rtx_cnt++;
						}
					}
					else if ((timers[tgrcantx + tdevix] > 5) && (grcan_data_req == 0))
					{		 // Update outputs
					...
					if (sync_grc.hcb[iodev_ix].reflect_data == 1)
					{		 // Send back what was sent so the device can verify it was sent ok
						sync_grc.hcb[iodev_ix].reflect_data = 0;	// Must be cleared before data is packed - if new data comes in during
						grcan_debug_3++;
						pack_grcan_out(15,devnmb,0);			//  the pack, it must be re-sent
					}
					else 
					...
					else
					{
						...
						grcan_dev[devix].online = 0;
#if (Simulator == 0)
			 			serial_grc.hcb[iodev_ix].iob &= GIO_In_Out[iodataix];	  // clear input data since device is not on-line
#endif
					}
				}
