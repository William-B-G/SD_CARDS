...
/*
	...
	Device		  Number
     ...
     SEB_1	 		11		  Serial I/O Expansion board 1	(io brd 7) [1134 - wbg]
     SEB_2	 		12		  Serial I/O Expansion board 2	(io brd 8) [offset of 10, so Dip SW= 2 - wbg]
     ...
     PI_LANT 		20		  Serial PI and Lantern (global address for all pi and lantern devices) [Position indicator ? - wbg]
...
*/
		...
		case 13:		// I/O Board outputs
			...
			if ((devnmb >= c_ctcan_SEB_1) && (devnmb <= c_ctcan_SEB_40))
			{
				...
				if(IFL_IFB_debug)
			    {
			    	sprintf(src,"data_0 0x%X",ctcan_tx[ctc_tpix].data[0]);
			    	Add_msg_to_array_B(src, 1);
			    	sprintf(src,"data_1 0x%X",ctcan_tx[ctc_tpix].data[1]);
			    	Add_msg_to_array_B(src, 2);
			    	sprintf(src,"data_2 0x%X",ctcan_tx[ctc_tpix].data[2]);
			    	Add_msg_to_array_B(src, 3);	
			    	sprintf(src,"adr %X _ix %X, aix %X",ctcan_tx[ctc_tpix].addr, iodev_ix, iodataix );
			    	Add_msg_to_array_B(src, 4);
			    	
			    	IFL_IFB_debug = 0; // to disable and avoid overwrite			    			    				    	
			    }
				if (cons[RGB_CC] == 1)
				{
