...
{ // toggle LED Z6 if 'LED_Z6' true
    ...
    int16 ResetArray[] =
        {
            0, 2, 5, 8, 10, 11}; // last value (11) a dummy place holder
    ...
    case send_reset:
        if (timers[t_Generic_Delay] < ExtraDelay + 2 * Delay_Count)
            break;
        //  ADD CODE
...
int16 Send_Hall_Data(int16 msg)
{
    ...
    //    SIU.GPDO[Z0_LED].R = !SIU.GPDO[Z0_LED].R;  // toggle LED;
    if (Or_it)
        msg |= Latest_Hall_data; // to be able to turn one lantern
    ...
    case send_hall_data:
        ...
        if (No_Ack)
            ...
            test_Kase = send_reset_0;
        break;

    case send_reset_0:
        if (timers[t_Generic_Delay_3] < Delay_Count)
            break;
        //  ADD CODE
        timers[t_Generic_Delay_3] = 0;
        Send_Hall_Reset(0);
...
int16 Set_or_Clear_1137_Input(int16 Set_Clear)
{
    ...
    {
        if (chan_1)
            setoutp(Test_GALx1137_out_array[6]); // Returns 0x08
        if (chan_2)
            setoutp(Test_GALx1137_out_array[7]); // Returns 0x02
        if (chan_3)
            setoutp(Test_GALx1137_out_array[8]); // Returns 0x08
        if (chan_4)
            setoutp(Test_GALx1137_out_array[9]); // Returns 0x02
        if (chan_5)
            setoutp(Test_GALx1137_out_array[10]);
        if (chan_6)
            setoutp(Test_GALx1137_out_array[11]);
    ...
    else
    {
        ...
        if (chan_2)
            clroutp(Test_GALx1137_out_array[7]);
        if (chan_3)
            clroutp(Test_GALx1137_out_array[8]);
        if (chan_4)
            clroutp(Test_GALx1137_out_array[9]);
        if (chan_5)
            clroutp(Test_GALx1137_out_array[10]);
        if (chan_6)
            clroutp(Test_GALx1137_out_array[11]);
    }
...
int16 Qty_113x_bds(int16 Test_and_Qty)
{
    ...
    case initialize:
        ...
        if (The_Test == GALx1139)
            Direction = consB[27] & 0x01;
        else
            Direction = consB[27] & 0x10;
    ...
    case review_Qty:
        ...
        if (Direction)
            sprintf(src, "Dec, tL=%d, CL=%d", test_Loop, current_Loop);
        else
            sprintf(src, "Inc, tL=%d, CL=%d", test_Loop, current_Loop);
        //            Log_an_event_C(src);
        break;
...
int16 Hall_Station_CAN(int16 Test_loop)
{
    ...
    case Init_GRCAN_Test:
        ...
        if (Multi_board_test == FALSE)
            Clear_Msg_Board(); //  Clear_Log_Buffer();
        clr_lcd_dply();
...
*/
    case Verify_ON:
        ...
        {
            if (Hall_Output_1 == 0)
                Log_an_event("IO_1_OUT not ON     ");
            if (Hall_Output_2 == 0)
                Log_an_event("IO_2_OUT not ON     ");
            GRCAN_TestCase = Reset_2s_OnceMore;
        }
        timers[t_Generic_Delay] = 0;
        break;
    ...
    case Check_4_short_chan_2:
        if (timers[t_Generic_Delay] < 2 * Delay_Count)
            break;
...
*/
    ...
    case Verify_0:
        if (timers[t_Generic_Delay] < 3 * Delay_Count)
            break;
        timers[t_Generic_Delay] = 0;
        if (Data_Zero == 0)
        {
...
int16 Incr_Decr_10(int16 dummy) // delay_for_Comm_to_1137 Incr_Decr_10
{
    ...
    if (Check_for_Up_Button())
    {
        if (delay_for_Comm_to_1137 < 200)
            delay_for_Comm_to_1137 += 10;
        else
            delay_for_Comm_to_1137 += 100;
        if (delay_for_Comm_to_1137 > 600)
            delay_for_Comm_to_1137 = 600;
        sprintf(src, "delay = %d  ", delay_for_Comm_to_1137);
        New_write_display(src, 0, 0);
    ...
    if (Check_for_Down_Button())
    {
        if (delay_for_Comm_to_1137 < 200)
            ...
            delay_for_Comm_to_1137 -= 100;
        if (delay_for_Comm_to_1137 < 0)
            delay_for_Comm_to_1137 = 0;
        sprintf(src, "delay = %d  ", delay_for_Comm_to_1137);
        New_write_display(src, 0, 0);
        return CONTINUE_SERVICE;
    }
...
/*    
    ...
    {                // test io board
        ...
        if (rdinp(GALX4_0047_in_array[0])) // switch 1
        {
...
{ // Enable_gr_CAN_Ctrl
    if (enable_disable)
        Enable_gr_CAN_Ctrl = 1;
    else
        Enable_gr_CAN_Ctrl = 0;
