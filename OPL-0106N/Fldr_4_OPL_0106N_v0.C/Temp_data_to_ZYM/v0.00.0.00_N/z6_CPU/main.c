...
const int8 Sandbox_revision = 'N' ;  // Normally, set this = '0'. 
  // Then increment as 'A', 'B', etc
char *VerDate = "2018-03-19 @08:00";               
...
/*            
          // ZYM 12-7-2017  	
  		 if(Enable_CAN_Ctrl) 
    	 {        	
       		comm_control(); //normally called, but suspended while
             // doing CAN test so that CAN test can control CAN traffic.
    	 }                     
         if(Enable_iospi_Ctrl)
    	 {  	
 			iospi_control(); 
    	 }      
*/                       		  
          if(timers[tinc] & 0x01) // (timers[tinc] & 0x01) == 0 =>20ms
