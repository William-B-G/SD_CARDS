...
const int8 Sandbox_revision = 'C' ;  // Normally, set this = '0'. 
  // Then increment as 'A', 'B', etc    

////////////////////
   
//char *VerDate = "2018-09-02 @09:14";  //  A
//char *VerDate = "2018-09-08 @10:17";  //  B
char *VerDate = "2018-09-17 @11:31";  //  C
// Derived from GALX 1134_CTB test fixture code, ver 1.R (2018-5-08)
// which was derived from GALX 1137 / 1139 test fixture code, ver P17 (2018-5-10)
...
// 	Enable_Hallcom_Ctrl = DISABLED; // disable RS-485 until ready to test RS485
    Set_RS485_Baud(NINETEEN_DOT_2);
 	...
 	The_120VAC_Ptr = (void *)SEB_outputs_array[O_C120T_enum];
...
void SetupRTC(void)
{
    d.year   = (int8)consB[15];
    d.month  = (int8)consB[14];
    d.day    = (int8)consB[13];
    t.hour   = (int8)consB[12];
    t.minute = (int8)consB[11];
    local_setdate();
    local_settime();    
}
 ...
 *
 * char *VerDate = "2018-09-17 @11:31";  //  C
 *  v 0.0.0 - C (2018-9-17) - Fix a bug in Diagnostics/Read SW0-5 states (bug shows when you press
 *      ENTER twice and then MODE once).
 *      Add menu item to Set/Clear each output of GALX-1107 Output SEB (dip switch = 2)
 


 *  v 0.0.1 - M (2018-7-12) - Fixes a bug in the " const char *Failure_msgs[] ", by adding msg[19] 'BLANK MESSAGE' so
 *    as not to get an 'array out of bounds' problem, which was reseting the MCU when GIF {6} was answered as NO with 
 *    the Mode button. Modify GIF {6} so as not to wait for startup operator msg response.
 *  v 0.0.1 - N (2018-7-13) - Small mod to GIF {6} Generic test data.
 *  v 0.0.1 - R (2018-7-13) - Remove debug code from version "v 0.0.1 - Q".  The debug code added to 'Generic_Tester()'.
...
int16 Function_Name(int16 dummy)
{
 ...
 *
 ...
 /*******************************************************************************
  ...
  Hardware Requirements:
    GALx-1132AN MCU board with 4-line, 20 segment display.
